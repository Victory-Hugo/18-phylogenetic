...
四件事，结果全部写进同一张长表：
  ...
  1. 逐窗聚类。每个时间窗**先按覆盖度筛掉不可靠的类别再聚类**——不可靠的曲线若留在
     距离矩阵里，会改变其余类别的聚类树与簇归属。被筛掉的类别记为 Not Assessed。
  2. 跨窗标签对齐。层次聚类的簇编号是任意的，用匈牙利算法在相邻窗**共有的可用类别**
     上重排编号，membership 热图上的同一条色带才代表同一群类别。
  3. 同簇频率。每次从每个类别各抽 1 条后验曲线，重算特征并重新聚类，统计任意两个类别
     落进同一簇的频率。这是**给定距离度量、连接方式与簇数**之下的后验同簇概率，不含
     模型选择的不确定性；输入不含真后验时会退化为伪样本频率并在日志中写明。
  4. 降维坐标与合并分数。降维坐标（PCA 默认，可选 UMAP）逐时间段各一套供散点图使用。
     合并分数分成**两个**：水平分数（平均 log Ne 与波动幅度）与变化分数（净变化、平均
     速率、最大扩张速率、最大收缩速率），各自取所属特征的第一主成分得分。
     合成一个总分是行不通的：实测单一 PC1 的载荷几乎全落在变化类特征上（平均水平仅
     0.04），也就是说那张图根本不含水平信息，却被当成"综合"来读。
     事件时间不进任何一个分数——它是"什么时候"，不是"强弱"，混进强度分数没有意义。
     热图颜色用**逐时间段标准化**后的分数：同一时间 bin 内的类别之间做 z-score。实测
     跨时间的整体漂移（全表标准差 1.92）远大于同一时段的类别间差异（中位 0.84），
     不逐段标准化的话，颜色画的是"时间走势"而不是"谁和谁不一样"。原始分数照常写进表里。

产出：
    <output_dir>/⭐2-Cluster-And-Similarity.tsv
    <output_dir>/debug/…（仅 debug_mode = true）
"""
