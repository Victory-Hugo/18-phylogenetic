...
`/mnt/f/onedrive/文档（科研）/脚本/Download/18-phylogenetic/5-拆树PAML合树`的功能是将一棵大树拆分呈多个子树（其中包括骨架树和其他子树），并将这些子树进行合树，最终得到一个新的树文件。

之所以要拆分，是因为需要对子树进行`paml`分析，而`paml`分析需要输入一个树文件和一个序列文件。在面对大树时，`paml`无法处理过大的树文件，因此需要将大树拆分成多个子树进行分析。

# 需求

现在流程已经完成了拆树和合树的功能，所以进一步需要添加`paml`数据准备、`paml`配置文件准备、`paml`运行等功能，以实现从树文件和序列文件到最终的`paml`分析结果、分析结果中提取子树、子树合并成大树的完整流程。

# 具体要求

`paml`要求输入文件：
1. 树文件：`nwk`格式，但是需要在树文件的开头打上`空格 tip树 1`，例如`  901 1;`，代表有901个tip，1棵树。
2. 序列文件：`fasta`格式，包含序列数据(其中的ID严格匹配树文件中的tip名称，不能多也不能少)。
3. 配置文件：`ctl`格式，包含`paml`分析的参数设置。以`/mnt/f/onedrive/文档（科研）/脚本/Download/18-phylogenetic/5-拆树PAML合树/conf/baseml_mtDNA.ctl`为例。

# 程序

`paml`运行参考`/mnt/f/onedrive/文档（科研）/脚本/Download/18-phylogenetic/5-拆树PAML合树/script/paml运行模板.sh`，需要在该脚本的基础上进行修改，以实现自动化的`paml`分析流程。

`paml`输出的结果是一个文本文件。你可以通过如下代码解析：
...
最终，获得新的树文件。然后接入现有流程将每个用于分析了的子树进行合树，得到最终的树文件。临时文件放在`/mnt/f/onedrive/文档（科研）/脚本/Download/18-phylogenetic/5-拆树PAML合树/output/tmp/`目录下。
