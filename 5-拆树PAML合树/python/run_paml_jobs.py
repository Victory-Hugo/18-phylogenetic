...
from paml_common import PAML_RUN_STATUS_COLUMNS, load_rows, run_baseml_job, write_rows
...
def _run_single_job(row, baseml_bin: str, skip_existing: bool, logger, job_index: int, total_jobs: int):
    ...
    for attempt_name, attempt_ctl, overrides in attempts:
        logger.info("%s attempt=%s ctl=%s", baseml_subtree_id, attempt_name, attempt_ctl.name)
        if overrides:
    ...
    elapsed_seconds = time.monotonic() - start_time
    logger.info(
        "Finished %s (%d/%d) status=%s return_code=%s elapsed=%.1fs",
        baseml_subtree_id,
        job_index,
        total_jobs,
        status,
...
def run(manifest_path, baseml_bin, parallel_jobs, skip_existing=True, log_level="INFO"):
    ...
    logger.info("Running %d baseml jobs with parallel_jobs=%s", len(rows), parallel_jobs)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_map = {
            executor.submit(_run_single_job, row, baseml_bin, skip_existing, logger, index, total_jobs): row["baseml_subtree_id"]
