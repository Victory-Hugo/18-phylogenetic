#!/usr/bin/env python3
"""Generate merge-ready fake PAML outputs with preserved topology and ultrametric branch lengths."""
...
from paml_common import PAML_JOB_MANIFEST_COLUMNS, PAML_PARSE_STATUS_COLUMNS, PAML_RUN_STATUS_COLUMNS


FAKE_PAML_MANIFEST_COLUMNS = [
    "baseml_subtree_id",
    "source_tree_file",
    "analysis_tree_file",
    "tree_seed",
...
def _infer_outgroup(records, paml_summary_tsv: Path) -> str:
...
def _materialize_fake_ultrametric_tree(
    source_tree,
    tree_seed: int,
...
def run(
    ...
    if branch_length_model != "lognormal_then_extend_terminal":
        raise PipelineError(f"Unsupported paml.fake_branch_length_model: {branch_length_model}")
    ...
    logger = setup_logger("fake_paml_outputs", output_dir / "fake_paml_outputs.log", log_level)
    ...
    outgroup_tip_name = _infer_outgroup(records, paml_summary_tsv)
    ...
    for record in records:
        source_tree_path = paml_tree_dir / Path(record.paml_tree_file).name
        if not source_tree_path.exists():
            raise PipelineError(f"PAML subtree file not found: {source_tree_path}")
        source_tree = read_newick_tree(source_tree_path)
        validate_tree_against_expected_tip_set(source_tree, record.total_tip_names, record.paml_subtree_id, outgroup_tip_name)
        validate_rooted_binary_tree(source_tree, outgroup_tip_name)

        tree_seed = random_seed + (stable_hash_int(record.paml_subtree_id) % (2**31))
        fake_result = _materialize_fake_ultrametric_tree(
            source_tree=source_tree,
            tree_seed=tree_seed,
            branch_length_sigma=branch_length_sigma,
            min_branch_length=min_branch_length,
            ultrametric_tolerance=ultrametric_tolerance,
        )
        analysis_tree_file = analysis_tree_dir / f"{record.paml_subtree_id}.nwk"
        write_tree_file(fake_result["tree"], analysis_tree_file)

        manifest_rows.append(
            {
                "baseml_subtree_id": record.paml_subtree_id,
                "job_dir": "",
                "treefile": source_tree_path.as_posix(),
                "seqfile": "",
                "ctlfile": "",
                "outfile": "",
                "expected_tip_count": str(record.total_n_tips),
                "tip_hash": record.tip_hash,
                "seq_id_strategy": "fake",
            }
        )
        run_status_rows.append(
            {
                ...
                "return_code": "0",
                "status": "success",
                "detail": "Generated by fake_paml_outputs.py",
            }
        )
        parse_status_rows.append(
            {
                ...
                "tip_length_csv": "",
                "status": "success",
                "detail": "Ultrametric fake analysis tree generated directly from split subtree topology.",
            }
        )
        fake_rows.append(
            {
        ...
        logger.info(
            "Generated %s seed=%d n_edges=%d post_delta=%s",
            record.paml_subtree_id,
            fake_result["tree_seed"],
            fake_result["n_edges_randomized"],
            fake_result["post_row"]["delta"],
        )

    write_rows(manifest_rows, PAML_JOB_MANIFEST_COLUMNS, output_dir / "paml_job_manifest.tsv")
    write_rows(run_status_rows, PAML_RUN_STATUS_COLUMNS, output_dir / "paml_run_status.tsv")
    write_rows(parse_status_rows, PAML_PARSE_STATUS_COLUMNS, output_dir / "paml_parse_status.tsv")
    write_rows(fake_rows, FAKE_PAML_MANIFEST_COLUMNS, output_dir / "fake_paml_manifest.tsv")
    ...
    if backbone_only_enabled:
        ...
        fake_backbone = _materialize_fake_ultrametric_tree(
            source_tree=source_backbone_tree,
            tree_seed=backbone_seed,
            branch_length_sigma=branch_length_sigma,
            min_branch_length=min_branch_length,
            ultrametric_tolerance=ultrametric_tolerance,
        )
        ...
        write_rows(
            backbone_report_rows,
            BACKBONE_ULTRAMETRIC_REPORT_COLUMNS,
            backbone_analysis_dir / "backbone_ultrametric_report.tsv",
        )
...
def build_parser():
    parser = argparse.ArgumentParser(description="Generate fake ultrametric PAML outputs for merge-stage development.")
    parser.add_argument("--paml-summary-tsv", required=True, help="Path to paml_subtree_summary.tsv.")
    parser.add_argument("--paml-tree-dir", required=True, help="Directory containing split PAML subtree trees.")
