...
#   conf/2-beast.yaml           - 第 2 步 beast 配置
...
#   - 项目目录下存在 pipe/1-run_split_pipeline.sh、pipe/2-prepare_beast_xml.sh、
#     pipe/2-run_beast_xml.sh ... pipe/5-run_time_calib_pipeline.sh。
...
#   bash pipe/0-run_all_pipeline.sh --beast-config /path/to/custom_beast.yaml
#
# 退出码：
#   0   - 所有步骤成功完成
...
BEAST_CONFIG="$PROJECT_ROOT/conf/2-beast.yaml"
...
Options:
  ...
  --beast-config PATH          Config for step 2 beast
...
ui_section "Phylogenetic Pipeline Console UI" "Full workflow | split -> beast -> merge -> ultrastandard -> time calibration"
ui_kv "Split config" "$SPLIT_CONFIG"
ui_kv "BEAST config" "$BEAST_CONFIG"
...
run_stage "Step 2A/5" "beast | 生成 BEAST XML" \
    bash "$PROJECT_ROOT/pipe/2-prepare_beast_xml.sh" \
    --config "$BEAST_CONFIG"

run_stage "Step 2B/5" "beast | 运行 BEAST XML" \
    bash "$PROJECT_ROOT/pipe/2-run_beast_xml.sh" \
    --config "$BEAST_CONFIG"
