...
def _validate_tree_files(
    ...
    for row in actual_rows:
        ...
        subtree_id = row.get("beast_subtree_id") or row.get("target_subtree_id") or tree_path.stem
...
def _run_fast_validation(output_dir: Path, rooted_tree, outgroup_tip_name: str, max_tips: int, report_rows):
    ...
    actual_paml_rows = load_table(output_dir / "beast_subtree_summary.tsv")
    actual_paml_manifest_rows = load_table(output_dir / "beast_tree_manifest.tsv")
    ...
    required_files = [
        ...
        output_dir / "beast_subtree_summary.tsv",
        output_dir / "beast_tree_manifest.tsv",
    ]
    ...
    for row in actual_paml_manifest_rows:
        paml_manifest_by_id.setdefault(row["beast_subtree_id"], []).append(row)

    if set(target_by_id) != set(paml_by_target_id):
        report_rows.append(("target_vs_paml_target_ids", "FAIL", "target_subtree_summary and beast_subtree_summary target ids differ"))
    else:
    ...
    _validate_tree_files(actual_paml_rows, output_dir, "total_tip_names", "beast_tree_file", outgroup_tip_name, max_tips, report_rows, "paml")
    ...
    for target_id, paml_row in paml_by_target_id.items():
        ...
        if int(paml_row["total_n_tips"]) != len(total_tip_names):
            report_rows.append(("paml_summary_counts", "FAIL", f"{paml_row['beast_subtree_id']}: total_n_tips mismatch"))
            continue
        if paml_row["tip_hash"] != compute_tip_hash(total_tip_names):
            report_rows.append(("paml_summary_hash", "FAIL", f"{paml_row['beast_subtree_id']}: tip_hash mismatch"))
            continue
        if int(paml_row["local_anchor_n_tips"]) != len(local_anchor_tip_names):
            report_rows.append(("paml_anchor_counts", "FAIL", f"{paml_row['beast_subtree_id']}: local_anchor_n_tips mismatch"))
            continue
        if int(paml_row["core_target_n_tips"]) != len(core_target_tip_names):
            report_rows.append(("paml_core_counts", "FAIL", f"{paml_row['beast_subtree_id']}: core_target_n_tips mismatch"))
            continue
        if int(paml_row["global_backbone_n_tips"]) != len(global_backbone_tip_names):
            report_rows.append(("paml_backbone_counts", "FAIL", f"{paml_row['beast_subtree_id']}: global_backbone_n_tips mismatch"))
            continue
        if paml_row["outgroup_tip"] not in total_tip_names:
            report_rows.append(("paml_outgroup_presence", "FAIL", f"{paml_row['beast_subtree_id']}: outgroup missing from total_tip_names"))
            continue
        if set(backbone_tip_names_row) != set(global_backbone_tip_names):
            report_rows.append(("paml_backbone_consistency", "FAIL", f"{paml_row['beast_subtree_id']}: backbone_tip_names vs global_backbone_tip_names mismatch"))
            continue
        if set(target_tip_names) != set(core_target_tip_names):
            report_rows.append(("paml_target_consistency", "FAIL", f"{paml_row['beast_subtree_id']}: target_nonbackbone_tip_names vs core_target_tip_names mismatch"))
            continue
        if len(set(total_tip_names)) != len(total_tip_names):
            report_rows.append(("paml_total_tip_uniqueness", "FAIL", f"{paml_row['beast_subtree_id']}: duplicate tips in total_tip_names"))
            continue
        report_rows.append(("paml_summary_counts", "PASS", f"{paml_row['beast_subtree_id']}: counts/hash verified"))
        ...
        if subtree_design_row is None:
            ...
            subtree_design_row["beast_tree_file"] != paml_row["beast_tree_file"]
        ...
        manifest_rows = paml_manifest_by_id.get(paml_row["beast_subtree_id"], [])
        if len(manifest_rows) != len(total_tip_names):
            report_rows.append(("paml_manifest_counts", "FAIL", f"{paml_row['beast_subtree_id']}: manifest tip count mismatch"))
        else:
            manifest_tip_names = sorted(row["tip_name"] for row in manifest_rows)
            if manifest_tip_names != sorted(total_tip_names):
                report_rows.append(("paml_manifest_consistency", "FAIL", f"{paml_row['beast_subtree_id']}: manifest tips mismatch"))
            else:
                report_rows.append(("paml_manifest_consistency", "PASS", f"{paml_row['beast_subtree_id']}: manifest verified"))
...
def _run_strict_validation(
    ...
    _compare_rows(actual_paml_rows, expected_paml_rows, "beast_subtree_id", report_rows, "paml_summary")
    _compare_rows(actual_paml_manifest_rows, paml_manifest_rows, ["beast_subtree_id", "tip_name"], report_rows, "paml_manifest")
    ...
    _validate_tree_files(actual_paml_rows, output_dir, "total_tip_names", "beast_tree_file", outgroup_tip_name, int(max_tips), report_rows, "paml")
