...
def run(
    ...
    if outgroup_tip_name in (None, "", "null"):
        ...
        if len(unique_outgroups) != 1:
            raise PipelineError(
                f"Could not infer a unique outgroup tip from {split_output_dir / 'beast_subtree_summary.tsv'}: {unique_outgroups}"
            )
    ...
    if set(target_by_id) != set(paml_by_target_id):
        raise PipelineError("target_subtree_summary.tsv and beast_subtree_summary.tsv do not cover the same target ids.")
