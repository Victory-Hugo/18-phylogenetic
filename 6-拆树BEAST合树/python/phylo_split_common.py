...
class PamlSubtreeRecord:
    ...
    def to_row(self) -> Dict[str, object]:
        return {
            "beast_subtree_id": self.paml_subtree_id,
            "target_subtree_id": self.target_subtree_id,
            "target_root_node_id": self.target_root_node_id,
            ...
            "beast_tree_file": self.paml_tree_file,
        }
...
def prepare_rooted_tree(
    ...
    if pre_binarize_rooted_tree:
        ...
        if outgroup_tip_name is not None:
            ...
            was_binary = False
        # 统计输入树中的多分叉（polytomy）：>2 个子节点的内部节点。非二叉树（如 IQ-TREE
        # 折叠低支持枝得到的树）会在此被解析为二叉——每个多分叉用零长合成节点拆分，
        # 后续 BEAST 起始树会把这些零长枝抬升到 min_branch_length，保证不退化。
        multifurcation_count = sum(1 for clade in tree.get_nonterminals() if len(clade.clades) > 2)
        max_children = max((len(clade.clades) for clade in tree.get_nonterminals()), default=0)
        ...
        if multifurcation_count > 0:
            logger.info(
                "Input tree is non-binary: resolved %d multifurcation(s) (max %d children) into bifurcations "
                "with zero-length synthetic branches; wrote rooted binary tree to %s",
                multifurcation_count,
                max_children,
                rooted_tree,
            )
        elif was_binary:
...
def clean_generated_outputs(output_dir: Path) -> None:
    ...
    for relative_path in [
        ...
        "anchor_manifest.tsv",
        "beast_subtree_summary.tsv",
        "beast_tree_manifest.tsv",
        ...
        rows.append(
            {
                ...
                "beast_tree_file": record.paml_tree_file,
            }
        )
    return rows
...
def build_paml_subtrees(
    ...
    for index, target_record in enumerate(target_records, start=1):
        ...
        record = PamlSubtreeRecord(
            paml_subtree_id=f"beast_subtree_{index:04d}",
            ...
            paml_tree_file=Path("beast_subtrees", f"beast_subtree_{index:04d}.nwk").as_posix(),
        )
        records.append(record)
        for tip_name in record.backbone_tip_names:
            manifest_rows.append(
                {
                    "beast_subtree_id": record.paml_subtree_id,
                    "tip_name": tip_name,
                    "tip_role": "backbone",
                    "target_subtree_id": record.target_subtree_id,
