...
def build_int_matrix_cached(fasta_dir, fname, samples):
    ...
    for pos in range(seq_len):
        col = sub[:, pos]
        # 保留所有样本（包括 N = 255），用于计算多态性
        alleles = col
        # 检查是否有多态性（排除全部为 N 的情况）
        valid_alleles = alleles[alleles != 255]
        if valid_alleles.size < 2:
            continue
        if np.unique(valid_alleles).size >= 2:
            rows.append(alleles.astype(np.int16))
    if not rows:
...
def rho_sigma_fast(A: np.ndarray, block_size: int = 20):
    ...
    M = A.astype(np.int16)  # 使用 int16 以保存 255 值
    n_sites, n_samples = M.shape
    D = np.zeros((n_samples, n_samples), dtype=np.int32)
    valid_sites = np.zeros((n_samples, n_samples), dtype=np.int32)  # 追踪有效位点数
    
    for st in range(0, n_sites, block_size):
        Mb = M[st:st+block_size]
        # 创建掩码：排除含有 N (255) 的比较
        valid_mask = (Mb != 255)[:, :, None] & (Mb != 255)[:, None, :]
        # 计算差异，只在有效位点计数
        diff = (Mb[:, :, None] != Mb[:, None, :]).astype(np.int32) & valid_mask.astype(np.int32)
        D += diff.sum(axis=0)
        valid_sites += valid_mask.sum(axis=0)
    
    # 避免除以零
    valid_sites = np.where(valid_sites == 0, 1, valid_sites)
    
    iu = np.triu_indices(n_samples, 1)
    tri = D[iu]
    valid_tri = valid_sites[iu]
    
    # 计算平均距离（归一化）
    rho = (tri / np.where(valid_tri == 0, 1, valid_tri)).mean()
    # 计算标准差
    sigma = math.sqrt(((tri / np.where(valid_tri == 0, 1, valid_tri))**2).sum() / (tri.size**2))
    return rho, sigma
